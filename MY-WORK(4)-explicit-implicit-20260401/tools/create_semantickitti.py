...
def get_semantickitti_info(split: str) -> dict:
    ...
    for i_folder in fold_split[split]:
        for j in range(total_num[i_folder]):
            seq_str = str(i_folder).zfill(2)
            frame_str = str(j).zfill(6)

            data_list.append({
                'lidar_points': {
                    'lidar_path':
                    osp.join('sequences', seq_str, 'velodyne',
                             frame_str + '.bin'),
                    'num_pts_feats':
                    4
                },
                'pts_semantic_mask_path':
                osp.join('sequences', seq_str, 'labels',
                         frame_str + '.label'),
                'sample_idx':
                seq_str + frame_str,
                ...
                'images': {
                    'CAM2': {
                        'img_path':
                        osp.join('sequences', seq_str, 'image_2',
                                 frame_str + '.png'),
                    },
                    'CAM3': {
                        'img_path':
                        osp.join('sequences', seq_str, 'image_3',
                                 frame_str + '.png'),
                    },
                },
                ...
                # ---- 新增：标定文件路径 ----
                'calib_path':
                osp.join('sequences', seq_str, 'calib.txt'),
            })
